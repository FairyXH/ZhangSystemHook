package com.android.server.permission.jarjar.kotlin.ranges;

/* JADX INFO: compiled from: Ranges.kt */
/* JADX INFO: loaded from: classes2.dex */
@com.android.server.permission.jarjar.kotlin.Metadata(d1 = {"\u0000,\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u0006\n\u0002\b\t\n\u0002\u0010\u000b\n\u0002\b\u0003\n\u0002\u0010\u0000\n\u0000\n\u0002\u0010\b\n\u0002\b\u0005\n\u0002\u0010\u000e\n\u0000\b\u0002\u0018\u00002\b\u0012\u0004\u0012\u00020\u00020\u0001B\u0015\u0012\u0006\u0010\u0003\u001a\u00020\u0002\u0012\u0006\u0010\u0004\u001a\u00020\u0002¢\u0006\u0002\u0010\u0005J\u0011\u0010\u000b\u001a\u00020\f2\u0006\u0010\r\u001a\u00020\u0002H\u0096\u0002J\u0013\u0010\u000e\u001a\u00020\f2\b\u0010\u000f\u001a\u0004\u0018\u00010\u0010H\u0096\u0002J\b\u0010\u0011\u001a\u00020\u0012H\u0016J\b\u0010\u0013\u001a\u00020\fH\u0016J\u0018\u0010\u0014\u001a\u00020\f2\u0006\u0010\u0015\u001a\u00020\u00022\u0006\u0010\u0016\u001a\u00020\u0002H\u0002J\b\u0010\u0017\u001a\u00020\u0018H\u0016R\u000e\u0010\u0006\u001a\u00020\u0002X\u0082\u0004¢\u0006\u0002\n\u0000R\u000e\u0010\u0007\u001a\u00020\u0002X\u0082\u0004¢\u0006\u0002\n\u0000R\u0014\u0010\u0004\u001a\u00020\u00028VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b\b\u0010\tR\u0014\u0010\u0003\u001a\u00020\u00028VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b\n\u0010\t¨\u0006\u0019"}, d2 = {"Lkotlin/ranges/OpenEndDoubleRange;", "Lkotlin/ranges/OpenEndRange;", "", "start", "endExclusive", "(DD)V", "_endExclusive", "_start", "getEndExclusive", "()Ljava/lang/Double;", "getStart", "contains", "", "value", "equals", "other", "", com.android.server.oplus.osense.OsenseConstants.KEY_INTEGER_HASH, "", "isEmpty", "lessThanOrEquals", com.android.server.wm.ActivityTaskManagerService.DUMP_ACTIVITIES_SHORT_CMD, "b", "toString", "", "kotlin-stdlib"}, k = 1, mv = {1, 9, 0}, xi = 48)
final class OpenEndDoubleRange implements com.android.server.permission.jarjar.kotlin.ranges.OpenEndRange<java.lang.Double> {
    private final double _endExclusive;
    private final double _start;

    public OpenEndDoubleRange(double start, double endExclusive) {
        this._start = start;
        this._endExclusive = endExclusive;
    }

    /* JADX WARN: Multi-variable type inference failed */
    @Override // com.android.server.permission.jarjar.kotlin.ranges.OpenEndRange
    public /* bridge */ /* synthetic */ boolean contains(java.lang.Comparable comparable) {
        return contains(((java.lang.Number) comparable).doubleValue());
    }

    @Override // com.android.server.permission.jarjar.kotlin.ranges.OpenEndRange
    public java.lang.Double getStart() {
        return java.lang.Double.valueOf(this._start);
    }

    @Override // com.android.server.permission.jarjar.kotlin.ranges.OpenEndRange
    public java.lang.Double getEndExclusive() {
        return java.lang.Double.valueOf(this._endExclusive);
    }

    private final boolean lessThanOrEquals(double a, double b) {
        return a <= b;
    }

    public boolean contains(double value) {
        return value >= this._start && value < this._endExclusive;
    }

    @Override // com.android.server.permission.jarjar.kotlin.ranges.OpenEndRange
    public boolean isEmpty() {
        return this._start >= this._endExclusive;
    }

    public boolean equals(java.lang.Object other) {
        if (!(other instanceof com.android.server.permission.jarjar.kotlin.ranges.OpenEndDoubleRange)) {
            return false;
        }
        if (!isEmpty() || !((com.android.server.permission.jarjar.kotlin.ranges.OpenEndDoubleRange) other).isEmpty()) {
            if (!(this._start == ((com.android.server.permission.jarjar.kotlin.ranges.OpenEndDoubleRange) other)._start)) {
                return false;
            }
            if (!(this._endExclusive == ((com.android.server.permission.jarjar.kotlin.ranges.OpenEndDoubleRange) other)._endExclusive)) {
                return false;
            }
        }
        return true;
    }

    public int hashCode() {
        if (isEmpty()) {
            return -1;
        }
        return (java.lang.Double.hashCode(this._start) * 31) + java.lang.Double.hashCode(this._endExclusive);
    }

    public java.lang.String toString() {
        return this._start + "..<" + this._endExclusive;
    }
}
