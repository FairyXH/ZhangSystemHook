...
final class ProgramInfoCache {
    ...
    void updateFromHalProgramListChunk(android.hardware.broadcastradio.V2_0.ProgramListChunk chunk) {
        if (chunk.purge) {
            this.mProgramInfoMap.clear();
        }
        for (android.hardware.broadcastradio.V2_0.ProgramInfo halProgramInfo : chunk.modified) {
            android.hardware.radio.RadioManager.ProgramInfo programInfo = com.android.server.broadcastradio.hal2.Convert.programInfoFromHal(halProgramInfo);
            putInfo(programInfo);
        }
        for (android.hardware.broadcastradio.V2_0.ProgramIdentifier halProgramId : chunk.removed) {
            this.mProgramInfoMap.remove(com.android.server.broadcastradio.hal2.Convert.programIdentifierFromHal(halProgramId));
        }
        this.mComplete = chunk.complete;
    }

    java.util.List<android.hardware.radio.ProgramList.Chunk> filterAndUpdateFrom(com.android.server.broadcastradio.hal2.ProgramInfoCache other, boolean purge) {
        return filterAndUpdateFromInternal(other, purge, 100, 500);
    }

    java.util.List<android.hardware.radio.ProgramList.Chunk> filterAndUpdateFromInternal(com.android.server.broadcastradio.hal2.ProgramInfoCache other, boolean purge, int maxNumModifiedPerChunk, int maxNumRemovedPerChunk) {
        if (purge) {
            this.mProgramInfoMap.clear();
        }
    ...
    java.util.List<android.hardware.radio.ProgramList.Chunk> filterAndApplyChunk(android.hardware.broadcastradio.V2_0.ProgramListChunk chunk) {
        return filterAndApplyChunkInternal(chunk, 100, 500);
    }

    java.util.List<android.hardware.radio.ProgramList.Chunk> filterAndApplyChunkInternal(android.hardware.broadcastradio.V2_0.ProgramListChunk chunk, int maxNumModifiedPerChunk, int maxNumRemovedPerChunk) {
        if (chunk.purge) {
            this.mProgramInfoMap.clear();
        }
        java.util.Set<android.hardware.radio.RadioManager.ProgramInfo> modified = new android.util.ArraySet<>();
        for (android.hardware.broadcastradio.V2_0.ProgramInfo halProgramInfo : chunk.modified) {
            android.hardware.radio.RadioManager.ProgramInfo info = com.android.server.broadcastradio.hal2.Convert.programInfoFromHal(halProgramInfo);
        ...
        for (android.hardware.broadcastradio.V2_0.ProgramIdentifier halProgramId : chunk.removed) {
            android.hardware.radio.ProgramSelector.Identifier removedId = com.android.server.broadcastradio.hal2.Convert.programIdentifierFromHal(halProgramId);
            if (removedId != null && this.mProgramInfoMap.containsKey(removedId)) {
